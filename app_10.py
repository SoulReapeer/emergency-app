...
engine.setProperty('voice', voices[1].id)  # Typically sets a female voice
...
def voice_prompt_incident_type():
    """Voice interface for selecting incident type"""
    speak("Please say the type of emergency from these options:")
    speak(", ".join(incident_types.keys()))
    ...
    while True:
        ...
        for incident_type in incident_types:
            if incident_type in command:
                return incident_type
                
        speak("I didn't recognize that emergency type. Please try again.")

def voice_prompt_specific_incident(incident_type):
    """Voice interface for selecting specific incident"""
    options = incident_types[incident_type]
    speak(f"Please say the specific {incident_type} emergency from these options:")
    speak(", ".join([incident_display_names[opt] for opt in options]))
    ...
    while True:
        ...
        for option in options:
            if option in command or incident_display_names[option].lower() in command:
                return option
                
        speak("I didn't recognize that specific emergency. Please try again.")

def voice_prompt_general_info():
    """Voice interface for collecting general information"""
    responses = {}
...
def voice_prompt_incident_details(specific_incident):
    """Voice interface for collecting incident details"""
    questions = incident_questions.get(specific_incident, {})
    responses = {}
...
def handle_voice_incident():
    """Complete voice incident reporting flow"""
    speak("Emergency voice reporting activated.")
    
    incident_type = voice_prompt_incident_type()
    specific_incident = voice_prompt_specific_incident(incident_type)
    general_info = voice_prompt_general_info()
    incident_info = voice_prompt_incident_details(specific_incident)
    ...
    # Log the incident
    saved_file = log_incident_voice(incident_data)
    
    if saved_file:
        speak(f"Emergency reported and logged. {feedback}")
    else:
        speak("Emergency reported but couldn't save details. Help is coming!")

def log_incident_voice(incident_data):
    """Save voice-reported incident to file"""
    try:
        incident_number = get_next_incident_number()
        timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        filename = f"incident_voice_{incident_number:03d}_{timestamp}.txt"

        with open(filename, "w") as file:
            file.write(f"=== Voice-Reported Incident #{incident_number} ===\n")
            file.write(f"Reported at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
            file.write(f"Emergency Type: {incident_data['incident_type']}\n")
            file.write(f"Specific Incident: {incident_display_names.get(incident_data['specific_incident'], incident_data['specific_incident'])}\n\n")

            file.write("=== General Information ===\n")
            for key, value in incident_data['general_info'].items():
                file.write(f"{key}: {value}\n")

            file.write("\n=== Incident Details ===\n")
            for key, value in incident_data['incident_info'].items():
                file.write(f"{key}: {value}\n")

            file.write(f"\n=== Emergency Instructions ===\n{incident_data['feedback']}\n")

        return filename
    except Exception as e:
        print(f"Error saving voice incident: {e}")
        return None
...
def get_next_incident_number():
    ...
    for f in incident_files:
        if f.startswith("incident_voice_"):
            try:
...
def main():
    ...
    speak("Welcome to the Emergency Response System")
    ...
    while True:
        ...
        if choice == "1":
            ...
            speak("Thank you for using the Emergency Response System. Goodbye!")
            break
        else:
