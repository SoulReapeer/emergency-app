...
Reported at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"""
        ...
        for i, report in enumerate(incident_reports, 1):
            ...
            for key, value in report['general_info'].items(): content += f"{key}: {value}\n"
            content += "\n=== Incident Details ===\n"
            for key, value in report['incident_info'].items(): content += f"{key}: {value}\n"
            content += f"\n=== Emergency Instructions ===\n{report['feedback']}\n\n"
        
        if safe_write_file(filename, content): return filename
        return None
    except Exception as e:
...
def responder_management(incident_type, incident_id, location):
    ...
    if incident_type not in responder_management_prompts: return
    ...
    print("\nSelect status:")
    for i, option in enumerate(prompts["status_options"], 1): print(f"{i}. {option}")
    status_choice = validate_input(f"Enter status number (1-{len(prompts['status_options'])}): ",
        lambda x: x.isdigit() and 1 <= int(x) <= len(prompts["status_options"]))
    status = prompts["status_options"][int(status_choice) - 1]
    
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    responder_filename = os.path.join(CONFIG["RESPONDER_LOG_DIR"], f"responder_{incident_id:03d}_{timestamp}.txt")
...
"""
    ...
    return {"responding_unit": responding_unit, "destination": destination, "reason": reason, "status": status}
            ...
            if 0 <= incident_num < len(incident_files):
                ...
                while True:
                    ...
                    if action == '1':
                        ...
                        else: print("Cannot access responder management without incident type and location.")
                    elif action == '5':
...
def show_advanced_menu():
    while True:
        ...
        else: print("Invalid choice. Please enter a number between 1-7.")
...
def handle_single_incident():
    ...
    for i, category in enumerate(categories): print(f"{i+1}. {category.capitalize()}")
    ...
    for i, incident in enumerate(incidents): print(f"{i+1}. {incident_display_names.get(incident, incident)}")
...
def handle_reporter_mode():
    ...
    while True:
        ...
        if another.lower() != 'yes': break
    
    if incident_reports:
        if len(incident_reports) == 1:
            report = incident_reports[0]
            responses = {'general_info': report['general_info'], 'incident_info': report['incident_info']}
            saved_file = log_single_incident(responses, report['incident_type'], report['specific_incident'], report['feedback'])
        else: saved_file = log_multiple_incidents(incident_reports)
