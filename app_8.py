...
# Get Feedback Based On Incidents
...
# Get Incident Number And Log Incident
...
def log_multiple_incidents(incident_reports):
    """Save multiple incidents to a single file"""
    try:
...
def handle_single_incident():
    """Process one complete incident"""
    print("\n=== New Incident Report ===\n")
    general_info = ask_questions(general_questions)

    # [Rest of the handle_single_incident() function from the previous example...]
    # [Include all the category selection and question handling code]
...
def main():
    ...
    incident_reports = []  # Stores multiple incidents
    ...
    while True:
        ...
        # Ask to continue
        while True:
            another = input("\nReport another incident? (yes/no): ").lower()
    ...
    if incident_reports:
        ...
        if saved_file:
            print(f"\nAll incidents logged in {saved_file}")
            print("Help is dispatched for all cases!")

        # Print summary
        print("\n=== Incidents Summary ===")
        for i, report in enumerate(incident_reports, 1):
            print(
                f"{i}. {incident_display_names[report['specific_incident']]} at {report['general_info']['location']}")
