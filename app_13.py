...
os.makedirs(CONFIG["RESPONDER_LOG_DIR"], exist_ok=True)
...
# Categorized Responder Directory - NEW
...
# Responder Management System - NEW
responder_management_prompts = {
    "medical": {
        "unit_prompt": "Enter medical unit/team that responded (e.g., Ambulance Unit 12, ER Team Alpha): ",
        "destination_prompt": "Where was the patient sent (hospital/clinic name and department)? ",
        "reason_prompt": "Reason for transport (e.g., trauma care, cardiac emergency, burn treatment): ",
        "status_options": ["Admitted", "Under Treatment", "Discharged", "Transferred", "Deceased"]
    },
    "fire": {
        "unit_prompt": "Enter fire unit/station that responded (e.g., Fire Station 4, Engine Company 2): ",
        "destination_prompt": "Where were victims/resources sent (e.g., shelter, hospital, command post)? ",
        "reason_prompt": "Reason for deployment (e.g., structure fire, vehicle fire, rescue operation): ",
        "status_options": ["Contained", "Under Control", "Extinguished", "Investigation Ongoing"]
    },
    "police": {
        "unit_prompt": "Enter police unit/station that responded (e.g., Precinct 5, SWAT Team Bravo): ",
        "destination_prompt": "Where were suspects/victims sent (e.g., police station, jail, court)? ",
        "reason_prompt": "Reason for response (e.g., robbery investigation, assault arrest, missing person): ",
        "status_options": ["In Custody", "Released", "Charged", "Under Investigation"]
    },
    "traffic": {
        "unit_prompt": "Enter traffic unit that responded (e.g., Highway Patrol Unit 3, Traffic Division): ",
        "destination_prompt": "Where were vehicles/victims sent (e.g., impound lot, hospital, repair shop)? ",
        "reason_prompt": "Reason for response (e.g., accident investigation, DUI arrest, road clearance): ",
        "status_options": ["Cleared", "Under Investigation", "Citation Issued", "Towed"]
    },
    "natural_disaster": {
        "unit_prompt": "Enter disaster response team (e.g., National Guard Unit, Red Cross Team): ",
        "destination_prompt": "Where were evacuees/resources sent (e.g., shelter, distribution center)? ",
        "reason_prompt": "Reason for deployment (e.g., flood response, earthquake relief, evacuation): ",
        "status_options": ["Rescue Complete", "Ongoing Operations", "Area Secured", "Recovery Phase"]
    },
    "hazardous_material": {
        "unit_prompt": "Enter HAZMAT team that responded (e.g., HAZMAT Unit 1, Environmental Response Team): ",
        "destination_prompt": "Where were materials/victims sent (e.g., disposal facility, decontamination unit)? ",
        "reason_prompt": "Reason for response (e.g., chemical spill, gas leak, biohazard cleanup): ",
        "status_options": ["Contained", "Cleanup Complete", "Ongoing Decontamination", "Area Secured"]
    },
    "special_situations": {
        "unit_prompt": "Enter special response team (e.g., SWAT Team, Bomb Squad, Crisis Negotiation): ",
        "destination_prompt": "Where were hostages/suspects sent (e.g., hospital, secure facility)? ",
        "reason_prompt": "Reason for deployment (e.g., hostage situation, bomb threat, active shooter): ",
        "status_options": ["Resolved", "Suspect Neutralized", "Ongoing Operation", "After Action Review"]
    },
    "cyber_incident": {
        "unit_prompt": "Enter cyber response team (e.g., Cyber Crime Unit, IT Security Team): ",
        "destination_prompt": "Where was evidence sent (e.g., forensic lab, secure server)? ",
        "reason_prompt": "Reason for response (e.g., data breach, ransomware attack, system intrusion): ",
        "status_options": ["Contained", "Under Investigation", "Systems Restored", "Vulnerability Patched"]
    },
    "utility_emergency": {
        "unit_prompt": "Enter utility response team (e.g., Power Company Crew, Water Department): ",
        "destination_prompt": "Where were repair resources sent (e.g., substation, pipeline location)? ",
        "reason_prompt": "Reason for response (e.g., power outage, gas leak, water main break): ",
        "status_options": ["Repair Complete", "Service Restored", "Ongoing Repair", "Assessment Phase"]
    },
    "weather_alert": {
        "unit_prompt": "Enter weather response team (e.g., Emergency Management, Storm Response): ",
        "destination_prompt": "Where were evacuees sent (e.g., shelter, emergency housing)? ",
        "reason_prompt": "Reason for response (e.g., hurricane evacuation, flood warning, tornado response): ",
        "status_options": ["Alert Lifted", "Ongoing Monitoring", "Recovery Operations", "Damage Assessment"]
    },
    "marine_incident": {
        "unit_prompt": "Enter marine response unit (e.g., Coast Guard Unit, Harbor Patrol): ",
        "destination_prompt": "Where were survivors/vessels sent (e.g., port, hospital, dry dock)? ",
        "reason_prompt": "Reason for response (e.g., boat capsizing, drowning, oil spill): ",
        "status_options": ["Rescue Complete", "Vessel Secured", "Ongoing Recovery", "Investigation Ongoing"]
    },
    "aviation_incident": {
        "unit_prompt": "Enter aviation response team (e.g., Airport Rescue, NTSB Team): ",
        "destination_prompt": "Where were survivors/wreckage sent (e.g., hospital, hangar, morgue)? ",
        "reason_prompt": "Reason for response (e.g., plane crash, emergency landing, mid-air incident): ",
        "status_options": ["Rescue Complete", "Investigation Ongoing", "Black Box Recovered", "Cleanup Complete"]
    },
    "public_health_incident": {
        "unit_prompt": "Enter public health team (e.g., CDC Response, Health Department): ",
        "destination_prompt": "Where were patients/samples sent (e.g., hospital, lab, quarantine)? ",
        "reason_prompt": "Reason for response (e.g., disease outbreak, contamination, poisoning): ",
        "status_options": ["Contained", "Under Treatment", "Monitoring", "Outbreak Controlled"]
    },
    "crowd_control": {
        "unit_prompt": "Enter crowd control unit (e.g., Riot Police, Event Security): ",
        "destination_prompt": "Where were arrested/injured persons sent (e.g., jail, hospital, processing)? ",
        "reason_prompt": "Reason for response (e.g., riot control, protest management, event security): ",
        "status_options": ["Situation Controlled", "Crowd Dispersed", "After Action Review", "Arrests Processed"]
    },
    "infrastructure_failure": {
        "unit_prompt": "Enter infrastructure response team (e.g., Engineering Corps, Repair Crew): ",
        "destination_prompt": "Where were repair resources sent (e.g., bridge site, tunnel entrance)? ",
        "reason_prompt": "Reason for response (e.g., bridge collapse, power grid failure, building integrity): ",
        "status_options": ["Repair Complete", "Structure Secured", "Ongoing Repair", "Assessment Complete"]
    }
}
...
# NEW: Responder Management System
def responder_management(incident_type, incident_id, location):
    """Handle responder dispatch and tracking for all incident types"""
    print(f"\n=== Responder Management for {incident_type.upper()} Incident ===")
    
    if incident_type not in responder_management_prompts:
        print("No specific responder management for this incident type.")
        return
    
    prompts = responder_management_prompts[incident_type]
    
    # Get responder information
    responding_unit = validate_input(prompts["unit_prompt"], validate_text)
    destination = validate_input(prompts["destination_prompt"], validate_text)
    reason = validate_input(prompts["reason_prompt"], validate_text)
    
    # Show status options
    print("\nSelect status:")
    for i, option in enumerate(prompts["status_options"], 1):
        print(f"{i}. {option}")
    
    status_choice = validate_input(
        f"Enter status number (1-{len(prompts['status_options'])}): ",
        lambda x: x.isdigit() and 1 <= int(x) <= len(prompts["status_options"])
    )
    status = prompts["status_options"][int(status_choice) - 1]
    
    # Log responder information
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    responder_filename = os.path.join(
        CONFIG["RESPONDER_LOG_DIR"], 
        f"responder_{incident_id:03d}_{timestamp}.txt"
    )
    
    responder_content = f"""=== Responder Management Log ===
Incident ID: {incident_id}
Incident Type: {incident_type}
Location: {location}
Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}

Responding Unit: {responding_unit}
...
"""
    ...
    if safe_write_file(responder_filename, responder_content):
        print(f"\nResponder management logged to {os.path.basename(responder_filename)}")
...
# NEW: Categorized Responder Selection
def show_categorized_responder_menu():
    """Display categorized responder role selection"""
    print("\n=== Select Your Response Category ===")
    
    # Get all categories that have responder roles
...
def display_incident_details(filename, responder_role, responder_category):
    ...
    # Extract incident type and location from file content
    incident_type = None
    location = None
    for line in content.split('\n'):
        if line.startswith("Emergency Type:"):
    ...
    if incident_type and incident_type in responders:
        ...
        # Check if responder's category matches the incident type
    ...
    while True:
        ...
        try:
            ...
            if 0 <= incident_num < len(incident_files):
                ...
                while True:
                    ...
                    if action == '1':
                        ...
                        if incident_type and location:
                            responder_info = responder_management(incident_type, incident_id, location)
                            if responder_info:
                                with open(filename, "a") as f:
                                    f.write(f"\n[RESPONDER MANAGEMENT] {datetime.now()}:\n")
                                    f.write(f"Unit: {responder_info['responding_unit']}\n")
                                    f.write(f"Destination: {responder_info['destination']}\n")
                                    f.write(f"Reason: {responder_info['reason']}\n")
                ...
                if action != '6':
                    display_incident_details(filename, responder_role, responder_category)
                return True
...
def handle_receiver_mode():
    ...
    while True:
        try:
            ...
            if not handle_responder_input(incident_files, responder_role, responder_category):
                break
